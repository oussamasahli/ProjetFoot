...
class dribleurplayerNV0(Strategy):
    ...
    def compute_strategy(self,state,id_team,id_player):
        ...
        if tools.estDansZone():

            if tools.PeutTirer(): 
                return SoccerAction(shoot = tools.VecPosGoal(10))
            else:
                return SoccerAction(tools.VecPosBall(10, maxBallAcceleration))

        else:

            if tools.PeutTirer(): 
                if  (((id_team==1) and (tools.PosBall().x >= GAME_WIDTH*2/3)) | ((id_team==2) and (tools.PosBall().x <= GAME_WIDTH/3))):

                    return SoccerAction(shoot = tools.VecPosGoal(0.7))
                else:
                    return SoccerAction(shoot = tools.VecPosGoal(0.2))
            else:
                return SoccerAction(tools.VecPosBall(10, maxBallAcceleration))
